...
import { CreateCustomerRequest, Customer, UpdateCustomerRequest } from "@shared/api";
...
export function SimpleAuthModal({ isOpen, onClose, onAuthSuccess }: SimpleAuthModalProps) {
  ...
  // const response = generateOtp(mobileNumber);
  ...
  const apiKey = 'd1f1acc8-7406-11f0-a562-0200cd936042';
  const otp_template_name = 'OTP';
  ...
  const validateMobileNumber = (mobile: string) => {
    const validationFlag = mobile.length === 10 && /^[6-9]\d{9}$/.test(mobile);
  ...
  const sendOTP = async () => {
    const otp = '728209';
    const apiUrl = `https://2factor.in/API/V1/${apiKey}/SMS/${mobileNumber}/${otp}/${otp_template_name}`;
    if (!validateMobileNumber(mobileNumber)) {
      setError("Please enter a valid 10-digit mobile number");
      return;
    }
    let userExists = false;
    const custResponse = await api.customers.getCustomerByPhone(mobileNumber);
      if(custResponse.success && custResponse.data) {
        setCustomer(custResponse.data);
        setIsUserExists(true);
      }
      if (authMode === "signin" && !isUserExists) {
        setError("Account not found. Please register first.");
        setAuthMode("register");
        return;
      } else if (authMode === "register" && isUserExists) {
        ...
        setIsLoading(true);
        setError("");
        // const result = await fetch(apiUrl, config).then((result) => {
        //   console.log(`Sending OTP to ${result}`);
  ...
  const verifyOTP = async () => {
    ...
    try {
      ...
      if (authMode === "signin" && isUserExists) {
        let updateeCustomer: UpdateCustomerRequest = {
          id: customer.id,
          email: customer.email,
          phone: customer.phone,
          firstName: customer.name,
          lastName: customer.name,
          dateOfBirth: customer.joinDate,
          gender: "male",
          preferences: {
            notifications: {
              email: true,
        ...
        const custResponse = await api.customers.updateCustomer(updateeCustomer.id, customer);
        if(custResponse.success && custResponse.data) {
          setCustomer(custResponse.data);
      ...
      if (authMode === "register" && !isUserExists) {
        let customer: CreateCustomerRequest = {
          firstName: "user",
          lastName: "",
          email: "harish.erupothu@gmail.com",
          phone: mobileNumber,
          password: "",
          isActive: true
        }
        const custResponse = await api.customers.createCustomer(customer);
        if(custResponse.success && custResponse.data) {
          setCustomer(custResponse.data);
          onAuthSuccess(customer);
          onClose();
            ...
            <>
              ...
              <div className="bg-fresh-50 p-3 rounded-lg">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-fresh-700">Secure & Fast</span>
                </div>
          ...
          {authStep === "otp" && (
            <>
              <div className="text-center space-y-2">
                ...
                <Badge variant="outline" className="text-fresh-600 border-fresh-200">
