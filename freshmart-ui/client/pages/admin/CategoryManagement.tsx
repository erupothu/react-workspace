...
export default function CategoryManagement() {
    ...
    const loadCategories = async () => {
      try {
        ...
        if (response.success && response.data) {
          console.log('Successfully loaded categories:', response.data);
          ...
          const categoriesWithSubs = await Promise.all(
            response.data.map(async (category) => {
              try {
                ...
                console.log(`Subcategories for ${category.id}:`, subResponse);
                return {
                  ...category,
          ...
          console.warn('API response not successful or no data, using fallback:', response);
  ...
  const flatCategories = React.useMemo(() => getAllCategories(categories), [categories]);

  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-fresh-600 mx-auto mb-4"></div>
        ...
        <CardContent>
          <div className="space-y-2">
            {categories?.map((category, index) => (
              category.subcategories.length>0?<CategoryTreeItem
                key={category.id}
                category={category}
                level={0}
...
const handleSaveCategories = async (newCategory) => {
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              {category.productCount?category.productCount:0} products
            </p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                className={errors.slug ? "border-red-500" : "border-gray-400"}
              />
              {errors.slug && (
